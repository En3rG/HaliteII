...
def move_battling_ships(MyMoves):
    """
    MOVE SHIPS THAT COULD BE IN BATTLE WITH IN THE NEXT FIVE TURNS
    FIRST PLACE IN A HEAP BASE ON DISTANCE FROM THE CLOSEST ENEMY SECTION
    THEN MOVE EACH ON, FROM CLOSEST TO ENEMY FIRST (TO AVOID COLLISION)
    """
    battle_heap = []
...
def get_battling_ships_heap(MyMoves, ship_id, battle_heap):
    """
    GET SHIPS INFO (IF BATTLING WTTHIN THE NEXT FIVE TURNS, BASED ON DISTANCE)
    ...
    perimeter = MyCommon.get_section_with_padding(enemy_matrix,
                                                  ship_coords,
                                                  MyCommon.Constants.PERIMETER_CHECK_RADIUS,
                                                  pad_values=0)
    ...
    d_sectioned = MyCommon.get_section_with_padding(distances,
                                                    ship_section_coord,
                                                    MyCommon.Constants.SIZE_SECTIONS_RADIUS,
                                                    pad_values=0)
    v_sectioned = MyCommon.get_section_with_padding(values,
                                                    ship_section_coord,
                                                    MyCommon.Constants.SIZE_SECTIONS_RADIUS,
                                                    pad_values=0)
    ...
    docked_distance, docked_coord = closest_section_with_enemy(MyMoves, ship_id, move_now=False, docked_only=True)
    ...
    if enemy_section_point: ## AN ENEMY WAS FOUND (CLOSEST AND MOST ENEMY)
        ...
        if section_distance <= 1.5:
            ...
            over_thrust = None
            heapq.heappush(battle_heap, (docked_distance, section_distance, enemy_distance, ship_id, docked_coord, target_coord, over_thrust, strong_enough, enemy_val))
        else:
...
def check_if_strong_enough(MyMoves, middle_coord):
    """
    CHECK A SECTION, BASED ON COORDS PROVIDED, IF ITS STRONG ENOUGH TO ATTACK FORWARD
    """
    ...
    num_enemy_in_section = (v_enemy[
                        MyCommon.Constants.ATTACKING_RADIUS - MyCommon.Constants.STRONG_ENOUGH_RADIUS:\
                        MyCommon.Constants.ATTACKING_RADIUS + MyCommon.Constants.STRONG_ENOUGH_RADIUS + 1,
                        MyCommon.Constants.ATTACKING_RADIUS - MyCommon.Constants.STRONG_ENOUGH_RADIUS:\
                        MyCommon.Constants.ATTACKING_RADIUS + MyCommon.Constants.STRONG_ENOUGH_RADIUS + 1] == -1).sum()
...
def move_battle_heap(MyMoves, battle_heap):
    ...
    IF SHIPS ARE IN IMMINENT BATTLE, WE DETERMINE IF WE ARE MOVING FORWARD
    BASED ON STRONG ENOUGH (DETERMINED EARLIER) OR IF ITS DYING (SO JUST KAMIKAZE)
    """
    while battle_heap:
        docked_distance, section_distance, enemy_distance, ship_id, docked_coord, target_coord, over_thrust, strong_enough, enemy_val = heapq.heappop(battle_heap)
                ...
                if over_thrust is None:
                    ...
                    if strong_enough or ship_dying:
                        ...
                        thrust, angle = astar.get_thrust_angle_from_Astar(MyMoves,
                                                                          ship_id,
                                                                          docked_coord,
                                                                          target_distance=docked_distance,
                        ...
                        over_thrust = 10
                        new_target_coord = MyCommon.get_destination_coord(ship_coords,
                                                                          flip_angle,
                                                                          over_thrust,
                                                                          rounding=False)
                        thrust, angle = astar.get_thrust_angle_from_Astar(MyMoves,
...
def move_ships_towards_this_coord(MyMoves, ship_id, ship_task, _ship_task, backup_coord):
    ...
    for _ship_id in ships:
        ...
        if _ship_id != ship_id and _ship_id not in MyMoves.myMap.ships_moved_already:
            ...
            _thrust, _angle = astar.get_thrust_angle_from_Astar(MyMoves,
                                                                _ship_id,
                                                                backup_coord,
                                                                target_distance=_d,
                                                                target_planet_id=None)
...
def closest_section_with_enemy(MyMoves, ship_id, docked_only=False, move_now=False):
    ...
    CAN SET DOCKED ONLY TO FIND CLOSEST DOCKED ENEMY SHIPS
    """
