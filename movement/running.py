...
def find_enemy(MyMoves, ship_id):
    ...
    if section_distance <= 2: ## ENEMY IS CLOSE
        ...
        logging.debug("enemy_val: {} enemy_distance: {}".format(enemy_val,enemy_distance))
        ...
        d_sectioned = MyCommon.get_section_with_padding(distances, ship_section_coord, 2, 0)
        v_sectioned = MyCommon.get_section_with_padding(values, ship_section_coord, 2, 0)

        logging.debug("d_sectioned: {}".format(d_sectioned))
